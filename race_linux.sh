...
go run -race $(ls *.go|grep -v signals_windows) \
	-chansize=500 \
        ...
        -nzb="$nzbfile" -provider=~/Downloads/provider.json \
	-cd=/cache/nzbrex -cc=true \
	-debug=false -debugmemlim=false -debugcache=false -debugsharedcc=false -debugconnpool=false -debugworker=false -debugBUG=false \

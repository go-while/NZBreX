...
if [ ! -f "NZBreX.exe" ]; then
    echo "ERROR: Windows executable not created"
    exit 1
fi

echo "Windows executable built successfully: NZBreX.exe"

# Check dependencies (optional, for debugging)
if command -v x86_64-w64-mingw32-objdump &> /dev/null; then
    echo "DLL dependencies:"
    if ! x86_64-w64-mingw32-objdump -p "NZBreX.exe" > objdump_output.txt; then
        echo "  ERROR: objdump failed to analyze NZBreX.exe"
